    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white p-4 rounded-lg shadow mb-4">
            <form method="GET" action="{{ route('offers.index') }}" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 items-end">
                {{-- Country search with typing + MCCs in suggestions --}}
                <div>
                    <label for="country_name" class="block text-sm font-medium text-gray-700">Country</label>
                    <input id="country_name"
                           name="country_name"
                           list="country_search_list"
                           value="{{ request('country_name') }}"
                           placeholder="Type country name..."
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    <datalist id="country_search_list">
                        @foreach($countries as $country)
                            @php
                                $mccs = $countryMccSummary[$country->id] ?? null;
                            @endphp
                            <option value="{{ $country->name }}">
                                {{ $country->name }}@if($mccs) ({{ $mccs }})@endif
                            </option>
                        @endforeach
                    </datalist>
                </div>
