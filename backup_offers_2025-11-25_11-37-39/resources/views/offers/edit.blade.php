<x-app-layout>
    ...
    <div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-sm rounded-lg p-6">
            ...
            <form method="POST" action="{{ route('offers.update', $offer) }}" class="mt-4 space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="country_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}" @selected(old('country_id', $offer->country_id) == $country->id)>{{ $country->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="network_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach ($networks as $network)
                                <option value="{{ $network->id }}" @selected(old('network_id', $offer->network_id) == $network->id)>{{ $network->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="supplier_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach ($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" @selected(old('supplier_id', $offer->supplier_id) == $supplier->id)>{{ $supplier->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    <div>
                        ...
                        <select name="product_type_dropdown_item_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($productTypeOptions as $id => $label)
                                <option value="{{ $id }}" @selected(old('product_type_dropdown_item_id', $selectedProductTypeId) == $id)>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="known_hops_dropdown_item_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($knownHopsOptions as $id => $label)
                                <option value="{{ $id }}" @selected(old('known_hops_dropdown_item_id', $selectedKnownHopsId) == $id)>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="sender_id_supported_dropdown_item_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($senderIdOptions as $id => $label)
                                <option value="{{ $id }}" @selected(old('sender_id_supported_dropdown_item_id', $selectedSenderIdId) == $id)>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="charge_type" value="{{ old('charge_type', $offer->charge_type) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Price</label>
                        <input type="text" name="price" value="{{ old('price', $offer->price) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    <div>
                        ...
                        <input type="date" name="effective_date" value="{{ old('effective_date', optional($offer->effective_date)->format('Y-m-d')) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    </div>

                    <div class="flex items-center mt-6">
                        <input id="is_exclusive" name="is_exclusive" type="checkbox" value="1" class="h-4 w-4 text-indigo-600 border-gray-300 rounded" @checked(old('is_exclusive', $offer->is_exclusive))>
