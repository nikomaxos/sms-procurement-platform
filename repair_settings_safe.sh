...
# AppServiceProvider with admin Gate
...
# EventServiceProvider (login/logout -> auth_logs)
...
class RouteType extends Model { protected $fillable = ['name']; }
PHP
...
namespace App\Models;
use Illuminate\Database\Eloquent\Model;
class KnownHop extends Model { protected $fillable = ['name']; }
PHP
...
namespace App\Models;
use Illuminate\Database\Eloquent\Model;
class ChargeModel extends Model { protected $fillable = ['name']; }
PHP
...
# --- Migration for auth_logs (create if missing) ---
...
class UserController extends Controller
{
    ...
    public function destroy(User $user) {
        if (auth()->id() === $user->id) return back()->with('status','You cannot delete yourself.');
        $user->delete();
...
class ${NAME}Controller extends Controller
{
    ...
    public function store(Request \$request) {
        \$data = \$request->validate(['name'=>['required','string','max:100','unique:${NAME}s,name']]);
        $NAME::create(\$data);
        return back()->with('status','Saved.');
    }
    public function update(Request \$request, $NAME \$item) {
        \$data = \$request->validate(['name'=>['required','string','max:100', Rule::unique('${NAME}s','name')->ignore(\$item->id)]]);
        \$item->update(\$data);
...
mkdir -p resources/views/settings/users resources/views/settings/dropdowns resources/views/settings/logs
...
<x-app-layout>
    ...
    <div class="p-6 space-y-10">
        ...
        <section>
            ...
            <form method="POST" action="{{ route('settings.route-types.store') }}" class="mb-3 flex space-x-2">@csrf
            ...
            <table class="w-full text-sm">
                ...
                <tr class="border-b">
                    <td class="py-2 pr-4 w-full">
                        <form method="POST" action="{{ route('settings.route-types.update',$rt) }}" class="flex space-x-2">@csrf @method('PUT')
                    ...
                    <td class="py-2">
                        <form method="POST" action="{{ route('settings.route-types.destroy',$rt) }}" onsubmit="return confirm('Delete?')">@csrf @method('DELETE')
        ...
        <section>
            ...
            <form method="POST" action="{{ route('settings.known-hops.store') }}" class="mb-3 flex space-x-2">@csrf
            ...
            <table class="w-full text-sm">
                ...
                <tr class="border-b">
                    <td class="py-2 pr-4 w-full">
                        <form method="POST" action="{{ route('settings.known-hops.update',$kh) }}" class="flex space-x-2">@csrf @method('PUT')
                            <input name="name" value="{{ $kh->name }}" class="border rounded p-2 w-full">
                            <button class="px-3 py-2 bg-green-600 text-white rounded">Save</button>
                        </form>
                    </td>
                    <td class="py-2">
                        <form method="POST" action="{{ route('settings.known-hops.destroy',$kh) }}" onsubmit="return confirm('Delete?')">@csrf @method('DELETE')
                            <button class="px-3 py-2 bg-red-600 text-white rounded">Delete</button>
                        </form>
                    </td>
                </tr>
        ...
        <section>
            ...
            <form method="POST" action="{{ route('settings.charge-models.store') }}" class="mb-3 flex space-x-2">@csrf
            ...
            <table class="w-full text-sm">
                ...
                <tr class="border-b">
                    <td class="py-2 pr-4 w-full">
                        <form method="POST" action="{{ route('settings.charge-models.update',$cm) }}" class="flex space-x-2">@csrf @method('PUT')
                            <input name="name" value="{{ $cm->name }}" class="border rounded p-2 w-full">
                            <button class="px-3 py-2 bg-green-600 text-white rounded">Save</button>
                        </form>
                    </td>
                    <td class="py-2">
                        <form method="POST" action="{{ route('settings.charge-models.destroy',$cm) }}" onsubmit="return confirm('Delete?')">@csrf @method('DELETE')
                            <button class="px-3 py-2 bg-red-600 text-white rounded">Delete</button>
                        </form>
                    </td>
                </tr>
...
<x-app-layout>
    ...
    <div class="p-6">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead><tr class="text-left border-b">
                    <th class="py-2 pr-4">#</th><th class="py-2 pr-4">When</th><th class="py-2 pr-4">User</th><th class="py-2 pr-4">Event</th><th class="py-2 pr-4">IP</th><th class="py-2 pr-4">User Agent</th>
                </tr></thead>
                <tbody>
                @foreach($logs as $log)
                    <tr class="border-b">
...
# Ensure DatabaseSeeder calls DropdownSeeder once
if grep -q 'DropdownSeeder::class' database/seeders/DatabaseSeeder.php; then
  : # already present
else
  perl -0777 -pe 's/public function run\(\): void\s*\{\s*/public function run(): void {\n        $this->call(\\Database\\Seeders\\DropdownSeeder::class);\n/;' -i database/seeders/DatabaseSeeder.php
fi

# --- Routes block (replace between markers, no "use" imports) ---

ROUTES=routes/web.php
grep -q "BEGIN SETTINGS ROUTES" "$ROUTES" && \
  sed -i '/BEGIN SETTINGS ROUTES/,/END SETTINGS ROUTES/d' "$ROUTES" || true
...
echo "[*] Linting PHP…"
CHANGED=()
while IFS= read -r -d '' f; do CHANGED+=("$f"); done < <(git ls-files -o -m --exclude-standard app/ resources/ routes/ database/ 2>/dev/null | tr '\n' '\0')
# If not a git repo, lint the known files
if [[ ${#CHANGED[@]} -eq 0 ]]; then
  mapfile -t CHANGED < <(printf "%s\n" \
    app/Providers/AppServiceProvider.php \
    app/Providers/EventServiceProvider.php \
    app/Models/RouteType.php app/Models/KnownHop.php app/Models/ChargeModel.php app/Models/AuthLog.php \
    ...
    app/Http/Controllers/Settings/AuthLogController.php \
    routes/web.php \
    database/seeders/DropdownSeeder.php \
    || true)
fi
for f in "${CHANGED[@]}"; do run "php -l $f" >/dev/null; done
echo "[*] PHP lint OK"

# --- Make sure admin@example.com is admin (idempotent) ---

run "php -r 'require __DIR__.\"/vendor/autoload.php\"; \$app=require __DIR__.\"/bootstrap/app.php\"; \$app->make(Illuminate\\Contracts\\Console\\Kernel::class)->bootstrap(); if(\$u=\\App\\Models\\User::where(\"email\",\"admin@example.com\")->first()){ \$u->is_admin=1; \$u->save(); echo \"Admin ensured\\n\"; } else { echo \"Admin not found (seed will create)\\n\"; }'"

# --- Clear caches, migrate, seed ---
...
echo "[+] Settings area ready. Go to /settings"
