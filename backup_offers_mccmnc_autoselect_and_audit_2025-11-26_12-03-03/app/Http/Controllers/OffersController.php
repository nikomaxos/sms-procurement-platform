...
class OffersController extends Controller
{
    ...
    /**
     * Αν ο χρήστης δεν επέλεξε MCCMNC αλλά το δίκτυο έχει ακριβώς ένα,
     * συμπλήρωσέ το αυτόματα στο request.
     */
    protected function ensureNetworkMncForRequest(Request $request): void
    {
        // Αν δεν υπάρχει network_id δεν κάνουμε τίποτα
        if (! $request->filled('network_id')) {
            return;
        }

        // Αν ήδη έχει σταλεί network_mnc_id, δεν το πειράζουμε
        if ($request->filled('network_mnc_id')) {
            return;
        }

        // Βρες όλα τα MNCs για το συγκεκριμένο δίκτυο
        $mncIds = NetworkMnc::where('network_id', $request->input('network_id'))
            ->pluck('id');

        // Αν υπάρχει ακριβώς ένα, σπρώξτο στο request
        if ($mncIds->count() === 1) {
            $request->merge([
