...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_country_input_and_4cols_$(date +%F_%H-%M-%S)"
...
<x-app-layout>
    ...
    @php
        ...
        $countryNameById = $countries->pluck('name', 'id');
        $countryLookup = $countries->mapWithKeys(function ($c) {
            return [$c->name => $c->id];
        });
    @endphp
    ...
    <div class="py-6 w-full max-w-full px-2 sm:px-4 lg:px-6 mx-auto">
        ...
        <form method="GET"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {{-- Country: single textfield with datalist (search while typing) --}}
                <div>
                    ...
                    <input
                        ...
                        value="{{ $selectedCountryId ? ($countryNameById[$selectedCountryId] ?? '') : '' }}"
                    >
                    ...
                    <input type="hidden" id="country_id" name="country_id" value="{{ $selectedCountryId }}">
                    <p class="mt-1 text-xs text-gray-500">
                        Start typing to search. Only one country can be selected.
                    </p>
                </div>
    ...
    <script>
        // Map country name -> id (exact match)
        document.addEventListener('DOMContentLoaded', function () {
            const mapping = @json($countryLookup);
            const input   = document.getElementById('country_search');
            const hidden  = document.getElementById('country_id');
            if (!input || !hidden) return;

            function syncIdFromLabel() {
                const label = input.value;
                if (mapping[label]) {
                    hidden.value = mapping[label];
                } else {
                    hidden.value = '';
                }
            }

            input.addEventListener('change', syncIdFromLabel);
            input.addEventListener('blur', syncIdFromLabel);
        });
    </script>
</x-app-layout>
BLADE
...
#    - 4 fields per row (σταθερά, όχι responsive 1-col)
###############################################
