...
BACKUP_DIR="$ROOT/backup_fix_version_snapshot_tar_${TIMESTAMP}"
LOG_DIR="$ROOT/logs"
LOG_FILE="$LOG_DIR/fix_version_snapshot_tar_${TIMESTAMP}.log"

SNAPSHOT_SCRIPT="$ROOT/scripts/version_snapshot.sh"
...
if [ -f "$SNAPSHOT_SCRIPT" ]; then
  cp "$SNAPSHOT_SCRIPT" "$BACKUP_DIR/version_snapshot.sh"
  echo "   [backup] $SNAPSHOT_SCRIPT -> $BACKUP_DIR/version_snapshot.sh"
else
  echo "   [info] No existing scripts/version_snapshot.sh found; creating a fresh one."
fi

echo "==> Writing new scripts/version_snapshot.sh (host+container aware, better tar excludes)..."

cat > "$SNAPSHOT_SCRIPT" << 'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
# ---------- DB settings from environment / .env ----------------------------
...
if [ $TAR_RC -ne 0 ]; then
  echo "WARNING: tar returned exit code $TAR_RC; continuing because archive may still be usable."
fi
...
echo "==> New scripts/version_snapshot.sh written."
echo "==> Backup of previous script (if any) is in: $BACKUP_DIR"
echo "   To restore old script (if needed):"
echo "   cp \"$BACKUP_DIR/version_snapshot.sh\" \"$SNAPSHOT_SCRIPT\""
