...
# -------------------------------------------------------------------
# Detect how to run artisan (host vs Docker)
# -------------------------------------------------------------------
ARTISAN="php artisan"
...
# -------------------------------------------------------------------
# 1) Patch navigation.blade.php to add admin-only Carriers Import
#    inside the Settings dropdown (Manage Account section).
# -------------------------------------------------------------------
...
if [ -f "$NAV_FILE" ]; then
  ...
  # Normalize any old is_admin checks to usertype === 'admin'
  ...
  if grep -q "Carriers Import" "$NAV_FILE"; then
    echo "==> Carriers Import already present in navigation; leaving as-is." | tee -a "$LOG_FILE"
  else
    echo "==> Injecting Carriers Import into Settings dropdown" | tee -a "$LOG_FILE"

    # First, try to insert before the Jetstream <!-- Authentication --> marker
    perl -0pi -e '
      if (index($_, "Carriers Import") == -1 && index($_, "<!-- Authentication -->") != -1) {
        s|(<!-- Authentication -->)|@if(auth()->check() && auth()->user()->usertype === '\''admin'\'')
                <div class="border-t border-gray-200 my-2"></div>
                <div class="block px-4 py-2 text-xs text-gray-400">
                    {{ __("Admin Tools") }}
                </div>
                <x-dropdown-link href="{{ route("carriers.import") }}">
                    {{ __("Carriers Import") }}
                </x-dropdown-link>
            @endif
    ...
    # If for some reason the comment is missing, try inserting before </x-slot>
    if ! grep -q "Carriers Import" "$NAV_FILE"; then
      perl -0pi -e '
        if (index($_, "Carriers Import") == -1 && index($_, "<x-slot name=\"content\">") != -1) {
          s|(</x-slot>)|@if(auth()->check() && auth()->user()->usertype === '\''admin'\'')
                <div class="border-t border-gray-200 my-2"></div>
                <div class="block px-4 py-2 text-xs text-gray-400">
                    {{ __("Admin Tools") }}
                </div>
    ...
    if grep -q "Carriers Import" "$NAV_FILE"; then
      echo "   - Carriers Import entry successfully added to Settings dropdown." | tee -a "$LOG_FILE"
    else
      echo "   - WARN: Could not inject Carriers Import into navigation (no expected anchors found)." | tee -a "$LOG_FILE"
    fi
  fi
else
...
# -------------------------------------------------------------------
# 2) Add Blade guard: only admin can access /carriers/import
# -------------------------------------------------------------------
