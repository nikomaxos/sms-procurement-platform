...
echo "==> Step4b_v3: countries.update fix + duplicate-MCC UI + flash log"
...
# 2) CountryMccController — user-friendly duplicate handling + log
b "$CMC"; mkdir -p "$(dirname "$CMC")"
cat > "$CMC" <<'PHP'
<?php
...
class CountryMccController extends Controller
{
    ...
    {
        $deleted = DB::table('country_mccs')->where([
            'country_id' => $country->id,
            'mcc'        => $mcc,
        ])->delete();
...
# 4) Include the flash log on countries/edit
if [ -f "$VC" ] && ! grep -q 'components/flash-log' "$VC"; then
  b "$VC"
  # Insert after first line to avoid complex quoting issues
  sed -i '1a@include("components.flash-log")' "$VC"
fi

# 5) Point countries.update route to CountryUpdateProxy (__invoke)
if [ -f "$R" ]; then
  b "$R"
  perl -0777 -i -pe 's|Route::put\(\s*["\x27]/countries/\{country\}["\x27]\s*,\s*\[\s*\\\\App\\\\Http\\\\Controllers\\\\CountriesController::class\s*,\s*["\x27]update["\x27]\s*\]\s*\)\s*->\s*name\(\s*["\x27]countries\.update["\x27]\s*\)\s*;|Route::put("/countries/{country}", [\\App\\Http\\Controllers\\CountryUpdateProxy::class, "__invoke"])->name("countries.update");|g' "$R"
fi

# 6) Lint & warm inside container
$DC exec -T app sh -lc '
  set -Eeuo pipefail
