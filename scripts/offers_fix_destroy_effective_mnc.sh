...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_fix_destroy_effective_mnc_$(date +%F_%H-%M-%S)"

echo "==> Backup dir: ${BACKUP_DIR}"
mkdir -p "${BACKUP_DIR}/app/Http/Controllers" "${BACKUP_DIR}/resources/views/offers"

# 0) Safety backups of current broken state
if [ -f "app/Http/Controllers/OffersController.php" ]; then
  cp app/Http/Controllers/OffersController.php "${BACKUP_DIR}/app/Http/Controllers/" || echo "WARN: could not backup current OffersController.php"
fi

if [ -f "resources/views/offers/edit.blade.php" ]; then
  cp resources/views/offers/edit.blade.php "${BACKUP_DIR}/resources/views/offers/" || echo "WARN: could not backup current edit.blade.php"
fi

###############################################
# 1) Restore clean OffersController from last
#    backup_offers_effective_mnc_and_destroy_*
###############################################
LATEST_EFFECTIVE_BACKUP="$(ls -dt backup_offers_effective_mnc_and_destroy_* 2>/dev/null | head -n 1 || true)"

if [ -z "${LATEST_EFFECTIVE_BACKUP}" ]; then
  echo "!! No backup_offers_effective_mnc_and_destroy_* directory found. Cannot safely restore controller."
  echo "Aborting without changes."
  exit 0
fi

if [ ! -f "${LATEST_EFFECTIVE_BACKUP}/app/Http/Controllers/OffersController.php" ]; then
  echo "!! Backup controller not found at ${LATEST_EFFECTIVE_BACKUP}/app/Http/Controllers/OffersController.php"
  echo "Aborting without changes."
  exit 0
fi

echo "==> Restoring OffersController.php from ${LATEST_EFFECTIVE_BACKUP}"
cp "${LATEST_EFFECTIVE_BACKUP}/app/Http/Controllers/OffersController.php" app/Http/Controllers/OffersController.php

###############################################
# 2) Add a single destroy() method if missing
###############################################
if grep -q "function destroy" app/Http/Controllers/OffersController.php; then
  echo "==> destroy() already exists in restored controller; leaving as is."
else
  echo "==> Adding destroy() method to OffersController"
  TMP_CTRL="${PROJECT_ROOT}/OffersController.tmp.$$"
  # Drop the final closing brace of the class
  sed '$d' app/Http/Controllers/OffersController.php > "${TMP_CTRL}"
  ...
  cat >> "${TMP_CTRL}" << 'PHP'
     ...
     */
    public function destroy(\App\Models\SupplierOffer $offer)
    {
        $offer->delete();
...
PHP
  ...
  mv "${TMP_CTRL}" app/Http/Controllers/OffersController.php
fi

###############################################
# 3) Rewrite edit.blade.php as unified
#    Create/Edit form with:
#    - MCC/MNC labels next to network
#    - Dynamic connection username/product/charge
...
<x-app-layout>
    ...
    @php
        ...
        // - old('effective_date') if validation failed
        // - else offer's effective_date if exists
        // - else today (for new offers)
    ...
    <div class="py-6 w-full max-w-full px-2 sm:px-4 lg:px-6 mx-auto">
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            >
                ...
                {{-- Effective Date (required, defaulted above) --}}
                <div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function applyConnectionDefaults() {
                ...
                // Αν ακόμη δεν βρέθηκε Product Type, ρώτα το server (τελευταία offer για το connection)
...
echo "==> Done. Controller restored + destroy() ensured, edit view rewritten."
echo "==> Backups of previous broken state at: ${BACKUP_DIR}"
