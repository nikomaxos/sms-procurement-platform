...
echo "==> [1/3] Ensure .env exists on HOST and has APP_KEY"
if [ ! -f .env ]; then
...
echo "==> [3/3] Verify from INSIDE Laravel (config('app.key'))"
$DC exec -T app sh -lc '
  set -Eeuo pipefail
  cd /var/www/html
...
echo $key ? "CFG_OK\n" : "CFG_MISS\n";
PHP
  php /tmp/check_app_key.php
'
echo "==> Done. Now hit /healthz and then a normal page."
