...
echo "==> Fixing countries/networks views (actions + CSS highlight)"

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

TS="$(date +%F_%H-%M-%S)"
BACKUP_ROOT=".backups/fix_networks_countries_${TS}"
mkdir -p "$BACKUP_ROOT"
...
backup_file() {
  ...
  if [ -f "$f" ]; then
    ...
    cp -a "$f" "${BACKUP_ROOT}/${safe}"
    echo "   - Backed up ${f} -> ${BACKUP_ROOT}/${safe}"
  else
    echo "   - WARN: ${f} not found, skipping backup"
  fi
}

echo "==> Backing up existing views"
backup_file "resources/views/countries/index.blade.php"
backup_file "resources/views/networks/index.blade.php"

echo "==> Writing resources/views/countries/index.blade.php"
...
echo "==> Writing resources/views/networks/index.blade.php"
...
echo "==> Attempting to clear & cache views via Docker"

# Prefer docker compose service name "app"
if docker compose ps app >/dev/null 2>&1; then
  if docker compose exec -T app php artisan view:clear && docker compose exec -T app php artisan view:cache; then
    echo "   - Views cleared & cached via docker compose exec app"
  else
    echo "   - WARN: artisan view commands failed via docker compose exec app"
  fi
else
  # Fallback: container name sms-platform-app if present
  if docker ps --format '{{.Names}}' | grep -q '^sms-platform-app$'; then
    docker exec -i sms-platform-app php artisan view:clear || true
    docker exec -i sms-platform-app php artisan view:cache || true
    echo "   - Views cleared & cached via docker exec sms-platform-app"
  else
    echo "   - WARN: could not detect running app container; please run 'php artisan view:clear && php artisan view:cache' manually inside the app container."
  fi
fi

echo "==> Done. Backups are in: ${BACKUP_ROOT}"
