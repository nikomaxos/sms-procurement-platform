...
###############################################################################
# 1) Make ITU fetch resilient (User-Agent + cURL fallback)
###############################################################################
...
###############################################################################
# 2) Networks index: reorder import controls + add pagination links
###############################################################################
...
###############################################################################
# 3) Networks edit: single confirmation on Save (if any deletions checked)
###############################################################################
# Remove per-row onclick and add submit-time confirm once
...
# Add single-confirmation logic on Save (only if any [delete] are checked)
awk '1; /@endsection|<\/x-app-layout>/ && !ins { print "<script>(function(){var f=document.querySelector('\"form[data-network-edit]\"'); if(!f){f=document.querySelector(\"form[action*='\\/networks']\")}; if(!f)return; f.addEventListener('submit',function(e){ var n=f.querySelectorAll(\"input[type=checkbox][name$='[delete]']:checked\").length; if(n>0){ if(!confirm('Remove '+n+' MNC(s)?')){ e.preventDefault(); } } });})();</script>"; ins=1 }' \
  resources/views/networks/edit.blade.php > resources/views/networks/edit.blade.php.tmp || true
if [ -s resources/views/networks/edit.blade.php.tmp ]; then
  mv resources/views/networks/edit.blade.php.tmp resources/views/networks/edit.blade.php
else
...
'
echo "Networks UI/import/pagination fixed."
