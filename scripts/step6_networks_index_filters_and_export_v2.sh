...
echo "==> Step6 v2: Networks index filters + CSV export + UI polish (robust patching)"

# --- 1) Patch NetworksController@index safely (no fragile quotes) ---
F=app/Http/Controllers/NetworksController.php
b "$F"
mkdir -p tools/patches

cat > tools/patches/patch_networks_index.php <<'PHP'
<?php
$F = 'app/Http/Controllers/NetworksController.php';
...
# run patcher inside container
$DC exec -T app php tools/patches/patch_networks_index.php

# --- 2) Replace networks/index.blade.php with filters + country typeahead + CSV ---
