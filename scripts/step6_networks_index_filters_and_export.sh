...
echo "==> Step6: Networks index filters + CSV export + UI polish"

############################################
# 1) Controller: enhance index()
############################################
...
if(strpos($c,"use Illuminate\\Http\\Request;")===false){
  $c=preg_replace("~^<\\?php\\s+namespace App\\\\Http\\\\Controllers;~",
...
public function index(Request \$request)
{
    ...
    // CSV export path (filtered)
    if (\$request->query('export') === 'csv') {
        \$rows = \$query->get(['id','country_id','name']);

        // Aggregate MCCs and MCC-MNC pairs per network_id for the exported set
    ...
    // Aggregate MCCs for current page only (lightweight)
...
if (preg_match($methodRx,$c)) {
  ...
  // Append method into class (before last closing brace)
  $c=preg_replace("~}\\s*\$~", $body . "\n}\n", $c, 1);
}
file_put_contents($F,$c);
echo "patched: NetworksController@index\n";
'

############################################
# 2) View: networks/index.blade.php
############################################
