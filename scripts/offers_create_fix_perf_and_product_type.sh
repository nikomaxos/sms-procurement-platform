...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_create_fix_$(date +%F_%H-%M-%S)"

echo "==> Backup dir: ${BACKUP_DIR}"
mkdir -p "${BACKUP_DIR}/resources/views/offers"

# Backup current create view
if [ -f "resources/views/offers/create.blade.php" ]; then
  ...
  echo "WARN: resources/views/offers/create.blade.php not found, will create a new one."
fi
...
<x-app-layout>
    ...
    <div class="py-6 w-full max-w-full px-2 sm:px-4 lg:px-6 mx-auto">
        ...
        @php
            // Απλή static λίστα για charge types για να μην κάνουμε βαριά queries από τη Blade
            $chargeTypeOptions = [
                'per_submit',
                'per_delivered',
            ];
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            <form method="POST" action="{{ route('offers.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {{-- Network (shortlist από χώρα + MCC/MNC δίπλα) --}}
                <div>
                ...
                {{-- Connection (οδηγεί default Product Type + Charge Type) --}}
                <div>
    ...
    {{-- JS: filter Networks by Country, MNCs by Network, και defaults από Connection (Product Type + Charge Type) --}}
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function applyConnectionDefaults() {
                ...
                // Charge Type: απλή αντιστοίχιση value
