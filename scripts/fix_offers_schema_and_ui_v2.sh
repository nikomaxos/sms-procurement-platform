...
echo "==> fix_offers_schema_and_ui_v2: starting"

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

STAMP="$(date +%F_%H-%M-%S)"
BACKUP_DIR=".backups/fix_offers_schema_and_ui_v2_${STAMP}"
...
# -------------------------------------------------------------------
# 1) Fix offers migrations (schema) so supplier_offers exists
#    - No FKs to views (countries, networks, network_mncs)
#    - Keep FKs only to real tables (suppliers, supplier_connections, supplier_offers)
# -------------------------------------------------------------------
...
# Move any duplicate 012636 migrations out of the way if they exist
for f in \
...
echo "==> Running migrations for offers..."
docker compose exec -T app php artisan migrate --force

# -------------------------------------------------------------------
# 2) Navigation: add "Offers" after Dashboard (desktop + mobile)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 3) Ensure bulk_update + history routes exist for Offers
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 4) Rebuild Offers index view:
#    - Filters packed into 2 rows on wide screens
#    - Bulk update panel appears only after "Mass Update" + selection
#    - Price & Effective Date link to history with tooltip for previous price
#    - Route Type column removed from UI (but DB column kept)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 5) Create/edit views: inherit Product Type & Charge Type from connection
#    + client-side guard: price < 1.000000 (0.xxxxxx)
# -------------------------------------------------------------------
...
echo "==> fix_offers_schema_and_ui_v2: done"
