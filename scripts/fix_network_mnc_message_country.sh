...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_networks_controller_msg_country_$(date +%F_%H-%M-%S)"

echo "==> Backup dir: ${BACKUP_DIR}"
mkdir -p "${BACKUP_DIR}/app/Http/Controllers"

if [ -f "app/Http/Controllers/NetworksController.php" ]; then
  cp app/Http/Controllers/NetworksController.php "${BACKUP_DIR}/app/Http/Controllers/NetworksController.php"
fi
...
class NetworksController extends Controller
{
    ...
    protected function syncMncs(Network $network, ?array $mncs): void
    {
        ...
        foreach ($normalized as $key => $row) {
            ...
            // Global uniqueness guard: MCC/MNC must not already exist on another network
            $globalExisting = NetworkMnc::where('mcc', $mccInt)
                ->where('mnc', $mncInt)
                ->where('network_id', '!=', $network->id)
                ->first();

            if ($globalExisting) {
                $otherNetwork = Network::with('country')->find($globalExisting->network_id);

                if ($otherNetwork) {
                    $countryName = optional($otherNetwork->country)->name;
                    if ($countryName) {
                        $otherLabel = $countryName . ' — ' . $otherNetwork->name;
                    } else {
                        $otherLabel = $otherNetwork->name;
                    }
                } else {
                    $otherLabel = 'unknown';
                }
                ...
                throw ValidationException::withMessages([
                    'mncs' => sprintf(
                        'MCC/MNC %03d/%03d is already assigned to another network (%s, ID %d). Remove it there first if you want to move it.',
                        $mccInt,
                        $mncInt,
                        $otherLabel,
                        $globalExisting->network_id
                    ),
                ]);
            }
