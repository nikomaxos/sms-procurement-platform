...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_create_perf_rework_$(date +%F_%H-%M-%S)"

echo "==> Backup dir: ${BACKUP_DIR}"
mkdir -p "${BACKUP_DIR}/app/Http/Controllers" "${BACKUP_DIR}/resources/views/offers" "${BACKUP_DIR}/routes"

# Backup controller, routes, create view
...
if [ -f "routes/web.php" ]; then
  cp routes/web.php "${BACKUP_DIR}/routes/" || echo "WARN: could not backup routes/web.php"
fi
...
#############################################
# 1) Append lightweight JSON endpoints in OffersController
#############################################

TMP_CTRL="${PROJECT_ROOT}/OffersController.tmp.$$"
# Κόβουμε την τελευταία γραμμή "}" του controller
sed '$d' app/Http/Controllers/OffersController.php > "${TMP_CTRL}"

cat >> "${TMP_CTRL}" << 'PHP'

    /**
     * Return MNCs (NetworkMnc records) for a given network as JSON.
     */
    public function networkMncsJson($network)
    {
        $mncs = \App\Models\NetworkMnc::query()
    ...
    /**
     * Return default product_type / charge_type for a given connection as JSON.
     *
     * Source of truth:
     *  1) SupplierConnection fields (if exist)
     *  2) Last SupplierOffer for that connection (as fallback)
     */
    public function connectionDefaultsJson($connection)
    {
        ...
        // 1) Try SupplierConnection
        $conn = \App\Models\SupplierConnection::find($connection);
        if ($conn) {
            // Charge type from connection, if present
            if (!empty($conn->charge_type)) {
                $defaults['charge_type'] = $conn->charge_type;
            }

            // Product type id / dropdown id from connection (υποθετικά πεδία)
            ...
            // Product type label from connection (π.χ. "Direct", "SS7")
            if (!empty($conn->product_type)) {
                $defaults['product_type_label'] = $conn->product_type;
            }
        }

        // 2) If still nothing useful, try last SupplierOffer for this connection
        if (empty($defaults['product_type_id']) && empty($defaults['product_type_label'])) {
            $offer = \App\Models\SupplierOffer::query()
...
#############################################
# 2) Append routes for the JSON endpoints
#############################################

cat >> routes/web.php << 'PHP'

Route::get('/offers/network/{network}/mncs-json', [\App\Http\Controllers\OffersController::class, 'networkMncsJson'])
    ->name('offers.network-mncs-json');

Route::get('/offers/connection/{connection}/defaults-json', [\App\Http\Controllers\OffersController::class, 'connectionDefaultsJson'])
    ->name('offers.connection-defaults-json');
PHP

#############################################
# 3) Rewrite Create Offer view to be lightweight + AJAX-driven
#############################################
