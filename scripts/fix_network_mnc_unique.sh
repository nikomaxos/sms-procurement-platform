...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_networks_controller_$(date +%F_%H-%M-%S)"
        ...
        foreach ($normalized as $key => $row) {
            ...
            if ($globalExisting) {
                throw ValidationException::withMessages([
                    'mncs' => sprintf(
                        'MCC/MNC %03d/%03d is already assigned to another network (ID %d). Remove it there first if you want to move it.',
                        $mccInt,
                        $mncInt,
