...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_unify_create_edit_$(date +%F_%H-%M-%S)"

echo "==> Backup dir: ${BACKUP_DIR}"
mkdir -p "${BACKUP_DIR}/app/Http/Controllers" "${BACKUP_DIR}/resources/views/offers"

# Backup OffersController & views
...
echo "==> Point create() to use offers.edit view instead of offers.create"

# Όπου ο controller κάνει render 'offers.create', το γυρνάμε σε 'offers.edit'
perl -pi -e "s/view\('offers.create'/view('offers.edit'/g" app/Http/Controllers/OffersController.php

echo "==> Rewriting resources/views/offers/edit.blade.php as unified Create/Edit form"
...
<x-app-layout>
    ...
    @php
        ...
        // Safe defaults for selected IDs in case controller δεν τα έστειλε
        ...
        // Charge type options από υπάρχουσες offers (μικρό query, όχι βαρύ)
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            >
                ...
                <div>
                    ...
                    @else
                        {{-- locked select, αλλά κρατάμε hidden field για να περάσει στο update --}}
                ...
                {{-- Connection (drives Product Type + Charge Type defaults) --}}
                <div>
                ...
                {{-- MNC (can always change; loaded via AJAX per network) --}}
                <div>
                ...
                {{-- Price (locked on edit, editable on create) --}}
                <div>
                ...
                {{-- Connection Username (read-only on edit, informational) --}}
                @if($isEdit)
                    <div>
    ...
    {{-- JS: filter networks by country, load MNCs via JSON, defaults from Connection --}}
    <script>
            ...
            function applyConnectionDefaults() {
                ...
                if (!productSet && connectionId) {
                    fetch(`${connDefaultsBase}/${connectionId}/defaults-json`)
                        ...
                        .catch(() => {
                            // σιωπηλή αποτυχία, ο χρήστης θα το βάλει χειροκίνητα
                        });
                }
            }
