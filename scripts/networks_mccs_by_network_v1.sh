...
BACKUP_DIR="$BACKUP_ROOT/networks_mccs_by_network_v1_${TS}"

mkdir -p "$LOG_DIR" "$BACKUP_DIR"

LOG_FILE="$LOG_DIR/networks_mccs_by_network_v1_${TS}.log"
...
echo "==> Rewriting networks index view so MCCs come from network->mncs only" | tee -a "$LOG_FILE"
...
echo "==> networks_mccs_by_network_v1.sh completed successfully." | tee -a "$LOG_FILE"
