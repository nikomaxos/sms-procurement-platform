...
BACKUP_DIR="backup_networks_reset_and_fix_all_$(date +%F_%H-%M-%S)"
...
for f in \
  ...
  resources/views/networks/create.blade.php
do
  if [ -f "$f" ]; then
    echo "==> Backing up $f"
...
########################################
# 1) Rebuild NetworksController
########################################
echo "==> Rewriting app/Http/Controllers/NetworksController.php"
...
########################################
# 2) Rewrite edit view (with delete button + MCC/MNC frame)
########################################
...
########################################
# 3) Create view (no $network usage)
########################################
echo "==> Rewriting resources/views/networks/create.blade.php"
...
########################################
# 4) Index view (green Create button + typeahead UX)
########################################
...
########################################
# 5) Syntax check (using service "app")
########################################
echo "==> php -l on NetworksController via docker compose exec app (if running)"

if docker compose ps app 2>/dev/null | grep -q 'Up'; then
  docker compose exec app sh -lc 'cd /var/www/html && php -l app/Http/Controllers/NetworksController.php && php artisan optimize:clear || true'
else
  echo "!! docker compose service \"app\" is not running; skipping container php check."
fi

echo "==> Done. Test /networks, /networks/create and /networks/{id}/edit in browser."
