...
[ -f docker/nginx/default.conf ] || cat > docker/nginx/default.conf <<'NGINX'
server {
  listen 80;
  server_name _;
...
[ -f .env ] || { cp -f .env.example .env 2>/dev/null || touch .env; echo "   - created .env"; }
...
echo "==> 2) Stop rescue stack (free port 8080)"
$DC -f docker-compose.clean.yml down || true

echo "==> 3) Write clean docker-compose.yml (reuse rescue DB volume)"
...
$DC exec -T app bash -lc '
  ...
  function mask(\$s){ if(!\$s){return "NULL";} return substr(\$s,0,10)."... (len=".strlen(\$s).")"; }

  echo "CONFIG_APP_KEY=".mask(\$cfg).PHP_EOL;
  echo "ENV_APP_KEY=".mask(\$env).PHP_EOL;
  echo (\$cfg ? "CFG_OK" : "CFG_MISS"), PHP_EOL;
  echo "MATCH_CFG_ENV=".( (\$cfg && \$env && \$cfg===\$env) ? "YES" : "NO" ), PHP_EOL;
  PHP
  php /tmp/check_app_key.php || true
  rm -f /tmp/check_app_key.php
'

echo "==> 6) (Optional) remove old node container"
docker rm -f sms-platform-node 2>/dev/null || true

echo "==> Done. Open: http://192.168.50.102:8080/healthz  then /login"
