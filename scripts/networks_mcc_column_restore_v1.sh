...
BACKUP_DIR="$BACKUP_ROOT/networks_mcc_column_restore_v1_${TS}"

mkdir -p "$LOG_DIR" "$BACKUP_DIR"

LOG_FILE="$LOG_DIR/networks_mcc_column_restore_v1_${TS}.log"
...
echo "==> Rewriting networks index view with MCC column restored" | tee -a "$LOG_FILE"
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                <div class="flex flex-wrap gap-1">
                                    @php
                                        $mccs = $network->country ? $network->country->mccs : collect();
                                    @endphp
                                    @forelse ($mccs as $mcc)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                                            {{ str_pad((string) $mcc->mcc, 3, '0', STR_PAD_LEFT) }}
                                        </span>
                                    @empty
                                        <span class="text-xs text-gray-400">—</span>
                                    @endforelse
...
echo "==> networks_mcc_column_restore_v1.sh completed successfully." | tee -a "$LOG_FILE"
