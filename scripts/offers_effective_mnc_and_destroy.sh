...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_effective_mnc_and_destroy_$(date +%F_%H-%M-%S)"

echo "==> Backup dir: ${BACKUP_DIR}"
mkdir -p "${BACKUP_DIR}/app/Http/Controllers" "${BACKUP_DIR}/resources/views/offers"

# Backup controller & edit view
...
########################################################
# 1) Add destroy() method to OffersController
########################################################

TMP_CTRL="${PROJECT_ROOT}/OffersController.tmp.$$"
# drop last line (the closing })
...
########################################################
# 2) Rewrite edit.blade.php to make:
#    - Effective Date required + default today on create
#    - MNC required
########################################################
...
<x-app-layout>
    ...
    @php
        ...
        if (!$defaultEffectiveDate) {
            $defaultEffectiveDate = optional($offer->effective_date)->format('Y-m-d') ?? now()->format('Y-m-d');
        }
    @endphp
    ...
    <div class="py-6 w-full max-w-full px-2 sm:px-4 lg:px-6 mx-auto">
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            >
                ...
                {{-- MNC (now required; loaded via JSON per network) --}}
                <div>
