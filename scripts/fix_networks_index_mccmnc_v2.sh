...
BACKUP_DIR="$BACKUP_ROOT/fix_networks_index_mccmnc_v2_${TS}"

mkdir -p "$LOG_DIR" "$BACKUP_DIR"

LOG_FILE="$LOG_DIR/fix_networks_index_mccmnc_v2_${TS}.log"
...
echo "==> Rewriting networks index view (no 'use' in @php; MCCMNC filter; MCCs per network)" | tee -a "$LOG_FILE"
...
echo "==> fix_networks_index_mccmnc_v2.sh completed successfully." | tee -a "$LOG_FILE"
