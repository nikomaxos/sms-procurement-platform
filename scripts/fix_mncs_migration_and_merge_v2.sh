...
echo "==> 1) Rewrite the add_network_mncs_and_audit migration WITHOUT creating the unique index"
...
echo "==> 2) Make sure the merge command exists (networks:merge-country-name)"
mkdir -p app/Console/Commands
if ! grep -q 'class MergeNetworksByCountryName' app/Console/Commands/MergeNetworksByCountryName.php 2>/dev/null; then
...
class MergeNetworksByCountryName extends Command {
    ...
    public function handle(): int {
        $this->info("Scanning duplicate Networks…");
...
echo "==> 3) Ensure the Console Kernel registers the command"
php -r '
...
echo "==> 4) Run migrations (no unique index yet)"
$DC exec -T app sh -lc 'php artisan migrate --force'

echo "==> 5) Merge duplicates now"
$DC exec -T app sh -lc 'php artisan networks:merge-country-name'

echo "==> 6) Add a NEW migration that only creates the unique index after merge"
ts="$(date +%Y_%m_%d_%H%M%S)"
idx_mig="database/migrations/${ts}_add_unique_networks_country_lowername.php"
cat > "$idx_mig" <<'PHP'
<?php
use Illuminate\Database\Migrations\Migration;
use Illuminate\Support\Facades\DB;
...
echo "==> 7) Run the unique-index migration"
$DC exec -T app sh -lc 'php artisan migrate --force'

echo "==> 8) Rebuild caches and ensure perms"
$DC exec -T app sh -lc 'php artisan optimize:clear && php artisan view:cache && php artisan route:cache && install -d -m 0777 storage/framework/{cache,sessions,views} bootstrap/cache && chmod -R 0777 storage bootstrap/cache'

echo "All fixed: migration passes, duplicates merged, unique index enforced."
