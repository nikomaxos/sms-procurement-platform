...
echo "Running rewrite_offers_index_clean_v1.sh..."

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

STAMP="$(date +%F-%H%M%S)"
BACKUP_DIR=".backups/rewrite_offers_index_clean_v1_${STAMP}"
...
if [ -f "$INDEX" ]; then
  ...
  echo "  - WARNING: $INDEX did not exist (will create a new one)"
fi

echo "  - Rewriting $INDEX with clean Blade template..."
            ...
            <div class="bg-white shadow rounded-lg mb-4">
                ...
                <div class="px-4 py-4 sm:px-6">
                    <form method="GET"
                        ...
                        <!-- Filters in responsive grid: 4–5 per row on wide screens -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                            ...
                            <!-- Charge Type (from connection/offer, not Charge Model) -->
                            <div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($offers as $offer)
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-3 py-2 whitespace-nowrap text-xs text-right text-gray-800">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function applyNetworkCountryFilter() {
                ...
                const currentValue = '{{ $filters['network_id'] ?? '' }}';
                if (currentValue) {
            ...
            function applyConnectionSupplierFilter() {
                ...
                const currentValue = '{{ $filters['supplier_connection_id'] ?? '' }}';
                if (currentValue) {
                    connectionFilter.value = currentValue;
                }
            }
...
echo "  - Clearing compiled Blade views (if possible)..."
...
echo "rewrite_offers_index_clean_v1.sh done."
