...
# 1) CountriesController: robust index() (no $mccs), solid edit() (passes $mccs)
CC="app/Http/Controllers/CountriesController.php"
backup "$CC"
# If the file exists, we surgically replace index() and edit(); if not, we create a minimal controller.
if [ -f "$CC" ]; then
  # Replace index()
  awk '
    BEGIN{inmethod=0}
    /public[ \t]+function[ \t]+index[ \t]*\(/ {print "/*__REPLACED_INDEX_START__*/"; 
      print "public function index(\\Illuminate\\Http\\Request $r)"; 
      print "{"; 
      print "    $per = max(1, min(1000, (int) $r->integer(\"per\", 20)));"; 
      print "    $countries = \\App\\Models\\Country::with(\"mccs\")->orderBy(\"name\",\"asc\")->paginate($per);"; 
      print "    return view(\"countries.index\", compact(\"countries\"));"; 
      print "}"; 
      print "/*__REPLACED_INDEX_END__*/"; 
      inmethod=1; next }
    inmethod && /\}/ { inmethod=0; next }
    !inmethod { print }
  ' "$CC" | awk '
    BEGIN{inmethod=0}
    /public[ \t]+function[ \t]+edit[ \t]*\([^{]*Country[ \t]*\$country[^{]*\)/ {
      print "/*__REPLACED_EDIT_START__*/";
      print "public function edit(\\App\\Models\\Country $country)";
      print "{";
      print "    $country->load(\"mccs\");";
      print "    $mccs = $country->mccs;";
      print "    return view(\"countries.edit\", compact(\"country\",\"mccs\"));";
      print "}";
      print "/*__REPLACED_EDIT_END__*/";
      inmethod=1; next
    }
    inmethod && /\}/ { inmethod=0; next }
    !inmethod { print }
...
# 2) NetworksController: fix ordering/loads; remove bad audit columns; fill MNC rows
...
class NetworksController extends Controller
{
    ...
    public function update(Request $r, \App\Models\Network $network)
    {
        ...
        // Determine primary MCC (network->primary_mcc if present; else first country MCC)
        $primaryMcc = (string)($network->primary_mcc ?? optional($network->country->mccs->first())->mcc ?? '');
        ...
        foreach ($mncs as $row) {
            ...
            $nm->mcc = $primaryMcc;
            $nm->mnc = $mnc;
            // No audit columns here (avoid missing-column errors)
            $nm->save();
        }
...
# 3) Networks index blade: clean header + actions
NB="resources/views/networks/index.blade.php"
backup "$NB"
cat > "$NB" <<'BLADE'
<x-app-layout>
  <x-slot name="header">
...
# 4) Networks edit blade: primary MCC read-only; show MNC rows; single confirm on save
...
<x-app-layout>
    ...
    <form method="POST" action="{{ route('networks.update', $network) }}" id="net-form" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white rounded-lg p-4 border">
        ...
        <div>
          ...
          <input value="{{ $network->primary_mcc ?? $network->country?->mccs?->first()->mcc }}" class="mt-1 w-full rounded border px-3 py-2 bg-gray-50" readonly>
        </div>
        <div>
          ...
          @foreach($rows as $i => $row)
            <div class="grid grid-cols-12 gap-2 items-center border rounded p-2">
              ...
              <div class="col-span-6">
                ...
                <input value="{{ ($network->primary_mcc ?? $network->country?->mccs?->first()->mcc ?? '') . ($row['mnc'] ?? '') }}" class="w-full rounded border px-2 py-1 bg-gray-50" readonly>
              </div>
  ...
  (function(){
    ...
    function currentMcc(){
      const el = document.querySelector('input[readonly][value^="{{ $network->primary_mcc ?? '' }}"]');
      return "{{ $network->primary_mcc ?? ($network->country?->mccs?->first()->mcc ?? '') }}";
    }
...
echo "Round 10: Countries index/edit fixed; Networks list actions, edit view, and update logic fixed."
