...
BACKUP_DIR="backup_networks_full_ui_fix_$(date +%F_%H-%M-%S)"
echo "==> Backup dir: $BACKUP_DIR"
mkdir -p "$BACKUP_DIR"

# Backup controller + views
for f in \
...
########################################
# 1) Add destroy() to NetworksController
########################################
echo "==> Patching NetworksController.php to add destroy()"

cat > scripts/_patch_networks_destroy.php << 'PHP'
<?php
...
$pattern = '/}\s*$/s';
$code = preg_replace($pattern, "\n" . $insert . "\n}\n", $code, 1, $count);

if (empty($count)) {
    fwrite(STDERR, "Could not locate class closing brace to inject destroy().\n");
    exit(1);
}

file_put_contents($file, $code);
fwrite(STDOUT, "destroy() added to NetworksController.\n");
PHP

php scripts/_patch_networks_destroy.php
rm -f scripts/_patch_networks_destroy.php

########################################
# 2) Rewrite edit.blade.php
########################################
echo "==> Rewriting resources/views/networks/edit.blade.php"
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white shadow sm:rounded-lg p-6 space-y-6">
            <form method="POST" action="{{ route('networks.update', $network->id) }}">
                ...
                {{-- Buttons row: Back + Save (update form only) --}}
            ...
            {{-- Separate delete form (cannot be nested inside update form) --}}
            <div class="mt-4 flex justify-between items-center">
                <span class="text-xs text-gray-500">
                    Danger zone: delete this network and its MCC/MNCs.
                </span>
                <form
                    method="POST"
                    action="{{ route('networks.destroy', $network->id) }}"
                    onsubmit="return confirm('Are you sure you want to delete this network ({{ $network->name }})?');"
                >
                    @csrf
                    @method('DELETE')
                    <button
...
########################################
# 3) Rewrite index.blade.php
########################################
echo "==> Rewriting resources/views/networks/index.blade.php"
...
<x-app-layout>
                ...
                >
                    ...
                    <div style="min-width: 260px; display:flex; justify-content:flex-end; gap:0.5rem;">
                        ...
                        <a
                            ...
                            class="inline-flex items-center px-3 py-2 border border-transparent text-xs font-semibold rounded-md shadow-sm text-white"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let items = Array.from(list.querySelectorAll('li[data-id]'));
            let activeIndex = -1;
            ...
            input.addEventListener('keydown', function (e) {
                ...
                if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                    ...
                    if (activeIndex >= 0 && activeIndex < vis2.length) {
                        e.preventDefault();
                        selectItem(vis2[activeIndex]);
                    }
                    // otherwise, let the form submit normally (e.g. pressing Enter in text field)
...
########################################
# 4) Syntax check + clear caches in container
########################################
...
echo "==> Done. Now test /networks and /networks/{id}/edit in browser."
