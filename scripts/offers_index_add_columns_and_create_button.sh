...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_index_cols_$(date +%F_%H-%M-%S)"
...
echo "==> Rewriting resources/views/offers/index.blade.php with:"
echo "   - Create Offer button"
echo "   - Conn. Username column"
echo "   - Last Edited / Edited By columns"
echo "   - Full width + horizontal scroll"
...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            ...
            {{-- Create Offer button (only if route exists) --}}
            @if (Route::has('offers.create'))
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse($offers as $offer)
                        <tr>
                            ...
                            <td class="px-3 py-2 whitespace-nowrap">
                                {{-- Προς το παρόν δείχνουμε id, αν υπάρχει στήλη updated_by_user_id --}}
