...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_offers_country_multiselect_and_layout_$(date +%F_%H-%M-%S)"
...
if [ -f "resources/views/offers/index.blade.php" ]; then
  cp resources/views/offers/index.blade.php "${BACKUP_DIR}/resources/views/offers/" \
    || echo "WARN: could not backup index.blade.php"
fi
...
###############################################
# 2) Patch OffersController country filter to support multi-select
###############################################
if grep -q "if (\$request->filled('country_id'))" app/Http/Controllers/OffersController.php; then
  echo "==> Patching country_id filter in OffersController@index to use whereIn..."
  perl -0pi -e "s/if\s*\(\s*\$request->filled\('country_id'\)\s*\)\s*\{\s*\$query->where\('country_id',\s*\$request->input\('country_id'\)\);\s*\}/if (\$request->filled('country_id')) {\n            \$countryIds = (array) \$request->input('country_id');\n            \$countryIds = array_filter(\$countryIds);\n            if (!empty(\$countryIds)) {\n                \$query->whereIn('country_id', \$countryIds);\n            }\n        }/s" app/Http/Controllers/OffersController.php
else
  echo "WARN: Could not find simple country_id filter block to patch. Controller left unchanged."
fi

###############################################
# 3) Rewrite offers/index.blade.php
#    - Country = multi select with type-ahead (single control)
#    - Keep all existing columns & features (last edited, edited by, etc.)
###############################################
...
<x-app-layout>
    ...
    @php
        $selectedCountryIds = collect(request('country_id', []))
            ->flatten()
            ->filter()
            ->map(fn ($v) => (int) $v)
            ->all();
    ...
    <div class="py-6 w-full max-w-full px-2 sm:px-4 lg:px-6 mx-auto">
        ...
        <form method="GET"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                <div>
                    <label for="country_id" class="block text-sm font-medium text-gray-700">Countries</label>
                    <select
                        id="country_id"
                        name="country_id[]"
                        multiple
                    ...
                    >
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}"
                                @selected(in_array($country->id, $selectedCountryIds, true))>
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
                    <p class="mt-1 text-xs text-gray-500">
                        Start typing to search; press Enter to toggle selection. Use Ctrl/Cmd+click for multiple.
                    </p>
                </div>
    ...
    <script>
        // Type-ahead search inside the multi-select for countries.
        document.addEventListener('DOMContentLoaded', function () {
            ...
            countrySelect.addEventListener('keydown', function (e) {
                // Enter toggles selection of the currently focused option (no form submit)
                if (e.key === 'Enter') {
                    e.preventDefault();
                    const idx = countrySelect.selectedIndex;
                    if (idx >= 0) {
                        const opt = countrySelect.options[idx];
                        opt.selected = !opt.selected;
                    }
                    return;
                }

                // Only handle printable characters (no ctrl/meta/alt combos)
                if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
                    ...
                    const options = Array.from(countrySelect.options).filter(o => o.value);
                    ...
                    for (let i = 0; i < options.length; i++) {
                        ...
                        if (o.text.toLowerCase().includes(searchTerm)) {
                            foundIndex = Array.from(countrySelect.options).indexOf(o);
                            break;
                        }
                    }
                    ...
                    if (foundIndex >= 0) {
                        ...
                        const opt = countrySelect.options[foundIndex];
                        const top = opt.offsetTop;
                        countrySelect.scrollTop = top - countrySelect.clientHeight / 2;
                    }
                }
            });
        });
...
###############################################
# 4) Rewrite offers/edit.blade.php
#    - 4 fields per row (grid-cols-4) for both create & edit
###############################################
