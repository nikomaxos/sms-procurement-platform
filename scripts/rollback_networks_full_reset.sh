...
BACKUP_DIR="backup_networks_manual_$(date +%F_%H-%M-%S)"
...
if [ -f "${VIEW}" ]; then
  echo "==> Backing up ${VIEW}"
  cp "${VIEW}" "${BACKUP_DIR}/networks_index.blade.php"
fi
...
class NetworksController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->query('export') === 'csv') {
            ...
            return response()->streamDownload(function () use ($rows, $map, $countriesMap) {
    ...
    protected function syncMncs(Network $network, ?array $mncs): void
    {
        ...
        // Check global uniqueness across networks and give friendly error
        if (!empty($normalized)) {
...
echo "==> Networks rollback/reset complete."
