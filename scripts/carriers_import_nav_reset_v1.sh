...
BACKUP_DIR="$BACKUP_ROOT/carriers_import_nav_reset_${TS}"

mkdir -p "$LOG_DIR" "$BACKUP_DIR"

LOG_FILE="$LOG_DIR/carriers_import_nav_reset_${TS}.log"
...
APP_LAYOUT_FILE="$ROOT_DIR/resources/views/components/app-layout.blade.php"
...
# -------------------------------------------------------------------
echo "==> Backing up Blade layout files..." | tee -a "$LOG_FILE"
backup_file "$NAV_FILE"
backup_file "$APP_LAYOUT_FILE"

# -------------------------------------------------------------------
# 2) Rewrite components/app-layout.blade.php
#    Standard Breeze-style layout that includes layouts.navigation
# -------------------------------------------------------------------
echo "==> Rewriting components/app-layout.blade.php..." | tee -a "$LOG_FILE"

mkdir -p "$(dirname "$APP_LAYOUT_FILE")"

cat > "$APP_LAYOUT_FILE" <<'BLADE'
<div class="min-h-screen bg-gray-100">
    {{-- Include primary navigation (left/basic menu + user dropdown) --}}
...
#    Left/basic menu: Dashboard + Carriers Import (admin only)
# -------------------------------------------------------------------
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- Carriers Import: visible only for admin usertype in basic menu --}}
                    @auth
...
echo "==> Blade files rewritten." | tee -a "$LOG_FILE"

# -------------------------------------------------------------------
# 4) Clear & rebuild Blade cache
# -------------------------------------------------------------------
...
if command -v docker-compose >/dev/null 2>&1 && [ -f "$ROOT_DIR/docker-compose.yml" ]; then
  if docker compose ps app >/dev/null 2>&1; then
    APP_SVC="app"
  elif docker compose ps sms-platform-app >/dev/null 2>&1; then
...
if [ -n "$APP_SVC" ]; then
  ...
  echo "   - docker compose / app service not detected; skipping artisan cache commands." | tee -a "$LOG_FILE"
fi
