...
-- 0) Drop networks unique index first to allow relinks/dedup
DO $$
DECLARE idx text;
BEGIN
  FOR idx IN
...
-- A) Resolve base table and prep (countries_base after first run)
DO $$
...
BEGIN
  ...
  -- Keep width 3 so 'n/a' reaches BEFORE trigger
...
-- C) Re-add strict CHECK (two letters) on VARCHAR(3) + unique
DO $$
DECLARE base_tbl text;
BEGIN
  base_tbl := CASE WHEN to_regclass('public.countries_base') IS NOT NULL
  ...
  -- Keep type varchar(3); enforce content with CHECK
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = base_tbl::regclass AND conname='countries_iso2_check')
  THEN EXECUTE format('ALTER TABLE %I DROP CONSTRAINT countries_iso2_check', base_tbl); END IF;
...
-- D) One-time table→view swap with INSTEAD OF triggers (no recursion)
DO $$
BEGIN
  IF to_regclass('public.countries_base') IS NULL THEN
    ...
    -- INSERT: UPSERT; keep "International" for ZZ
...
-- E) Ensure exactly one ZZ named International
DO $$
DECLARE base_tbl text;
BEGIN
  base_tbl := CASE WHEN to_regclass('public.countries_base') IS NOT NULL
...
-- F) Smoke test via VIEW: should coerce 'n/a' -> ZZ and keep International name
