...
echo "==> Apply guard + dedupe countries/networks, then add upsert rule"
$DC exec -T postgres psql -U app -d app -v ON_ERROR_STOP=1 <<'SQL'
BEGIN;

-- 0) Allow bad inputs to hit the BEFORE trigger
ALTER TABLE countries ALTER COLUMN iso2 TYPE varchar(3);

-- 1) Guard trigger: invalid -> 'ZZ', uppercase
...
-- 2) Drop STRICT check and unique index now (will recreate after dedupe)
DO $$
BEGIN
  IF EXISTS (
...
-- 3) Normalize existing rows via trigger
UPDATE countries SET iso2 = iso2;

-- 4) Dedupe countries by iso2 (keep lowest id); relink networks
...
-- 5) Normalize/dedupe networks on (country_id, lower(name))
-- 5a) Name normalizer BEFORE trigger (trim -> collapse spaces -> upper)
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM information_schema.tables WHERE table_name='network_mncs'
  ) INTO v_exists;
...
-- 6) Re-add strict CHECK (final row two uppercase letters) and full unique index
ALTER TABLE countries
  ADD CONSTRAINT countries_iso2_check CHECK (iso2 ~ '^[A-Z]{2}$');

CREATE UNIQUE INDEX countries_iso2_unique_idx ON countries(iso2);

-- 7) Upsert RULE for importer: plain INSERT -> INSERT ... ON CONFLICT (iso2) DO UPDATE
...
-- 9) Smoke test: this must not error; will coerce to ZZ then upsert
INSERT INTO countries(name, iso2, created_at, updated_at)
