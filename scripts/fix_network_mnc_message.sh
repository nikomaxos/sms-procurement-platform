...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR="${PROJECT_ROOT}/backup_networks_controller_msg_$(date +%F_%H-%M-%S)"
...
class NetworksController extends Controller
{
    ...
    protected function syncMncs(Network $network, ?array $mncs): void
    {
        ...
        foreach ($normalized as $key => $row) {
            ...
            if ($globalExisting) {
                $otherNetwork = Network::find($globalExisting->network_id);
                $otherName = $otherNetwork ? $otherNetwork->name : 'unknown';
