...
BACKUP_DIR="$BACKUP_ROOT/network_mnc_readonly_mcc_v1_${TS}"

mkdir -p "$LOG_DIR" "$BACKUP_DIR"

LOG_FILE="$LOG_DIR/network_mnc_readonly_mcc_v1_${TS}.log"
...
echo "==> Rewriting NetworksController (edit: load country.mccs)" | tee -a "$LOG_FILE"
...
echo "==> Rewriting networks edit view with MCC readonly + default from country.mccs" | tee -a "$LOG_FILE"
    ...
    <script>
        (function () {
            ...
            function addRow(mcc, mnc) {
                ...
                if (mcc === '' && !defaultMcc) {
                    // still allow empty, but keep readonly
                }
...
echo "==> network_mnc_readonly_mcc_v1.sh completed successfully." | tee -a "$LOG_FILE"
