...
### 0) Retire any legacy carriers:import command/closure without touching $commands
# Move any old files that declare the same signature out of the way
if [ -d app/Console/Commands ]; then
  while IFS= read -r -d '' f; do
    b "$f"; mv "$f" "$f.legacy.$ts.off" || true
  done < <(find app/Console/Commands -type f -name '*.php' -print0 | xargs -0 grep -l "carriers:import" 2>/dev/null || true)
fi
# Comment any routes/console.php closures that mention carriers:import
if [ -f routes/console.php ]; then
  b routes/console.php
  sed -i 's/\(carriers:import\)/\/\/ \1/g' routes/console.php || true
fi

### 1) Console Kernel — clean overwrite that registers our command
...
### 2) Importer Service (shared by CLI + UI)
...
class CarrierImportService {
    ...
    public function import(string $source, bool $fresh): array {
        ...
        // [Unverified] public datasets (fallback order)
        $sources = [
            'itu'    => ['https://raw.githubusercontent.com/musalbas/mcc-mnc-table/master/mcc-mnc-table.json'],
            'mccmnc' => ['https://raw.githubusercontent.com/musalbas/mcc-mnc-table/master/mcc-mnc-table.json'],
        ];
        ...
        DB::transaction(function() use (&$rows,&$processed,&$newCountries,&$newNetworks,&$newMncs){
            foreach ($rows as $row) {
                ...
                $exists = NetworkMnc::where('network_id',$network->id)->where('mcc',$mcc)->where('mnc',$mnc)->exists();
                if (!$exists) {
...
### 5) CountriesController (solid index/edit)
...
### 6) NetworksController (stable + MCC readonly derived from country)
...
### 7) NetworkMnc model — compute mcc_mnc (with safe Network import)
...
# countries/edit.blade.php (pluck()->implode())
...
# networks/index.blade.php (Create + Delete restored, per-page bottom-left)
...
# networks/create.blade.php — no partial, no $network usage
...
# networks/edit.blade.php — primary MCC read-only from country->mccs
...
### 9) Autoload + caches + confirm command
$DC exec -T app sh -lc '
