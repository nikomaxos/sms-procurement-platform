...
mkdir -p app/Console/Commands app/Http/Controllers resources/views/networks

###############################################################################
# 1) One-off backfill: legacy networks.mcc/mnc -> network_mncs if empty
###############################################################################
...
###############################################################################
# 2) Importer (non-destructive). Source selector (currently only ITU).
###############################################################################
...
###############################################################################
# 3) Controllers (Importer trigger + Networks CRUD with multi-MNC logic)
###############################################################################
...
class NetworksController extends Controller {
    ...
    public function index(Request $r){
        ...
        $q = Network::query()
            ->with(['mncs:id,network_id,mcc,mnc,mcc_mnc','country:id,name'])
            ->leftJoin('countries','countries.id','=','networks.country_id')
            ->addSelect('networks.*','countries.name as country_name')
            ->addSelect(DB::raw('(select MIN(m2.mcc_mnc) from network_mncs m2 where m2.network_id = networks.id) as min_mccmnc'))
            ...
            ->when($r->filled('mcc'), fn($qq)=>$qq->whereExists(function($sq) use($r){
                $sq->select(DB::raw(1))->from('network_mncs as nm')->whereColumn('nm.network_id','networks.id')->where('nm.mcc',$r->mcc);
            }))
            ->when($r->filled('mnc'), fn($qq)=>$qq->whereExists(function($sq) use($r){
                $sq->select(DB::raw(1))->from('network_mncs as nm')->whereColumn('nm.network_id','networks.id')->where('nm.mnc',$r->mnc);
            }))
            ->when($r->filled('mcc_mnc'), fn($qq)=>$qq->whereExists(function($sq) use($r){
                $sq->select(DB::raw(1))->from('network_mncs as nm')->whereColumn('nm.network_id','networks.id')->where('nm.mcc_mnc','ilike','%'.$r->mcc_mnc.'%');
            }))
    ...
    private function upsert(Request $r, Network $network){
        ...
        DB::transaction(function() use ($r, $network, $data) {
            ...
            foreach (($data['mncs_existing'] ?? []) as $id => $row) {
                ...
                if (isset($row['delete'])) { $m->delete(); continue; }
            ...
            foreach (($data['mncs_new'] ?? []) as $raw) {
                ...
                NetworkMnc::updateOrCreate(
                    ['mcc_mnc'=>$mccmnc],
                    ['network_id'=>$network->id,'mcc'=>$mcc,'mnc'=>$mnc,'updated_by_user_id'=>$user?->id,'updated_by_source'=>null]
                );
            }
        });
    ...
    private function primaryMccForCountry(?int $countryId, ?Network $network=null): string {
        ...
        if ($network && $network->relationLoaded('mncs') && ($first = $network->mncs->first()) && $first->mcc) {
            return (string)$first->mcc;
        }
        return '';
    }
...
###############################################################################
# 4) Views (index, edit form). No marked-for-deletion. Source dropdown + confirm.
###############################################################################
...
@php /* $network, $countries, $primaryMcc */ @endphp
...
###############################################################################
# 5) Run backfill and warm caches
###############################################################################
$DC exec -T app sh -lc '
