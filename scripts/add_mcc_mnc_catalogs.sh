...
# Networks create/edit with type-ahead
...
PHP;
if(strpos($s,"Countries & Networks (top-level)")===false){ $s.="\n".$block."\n"; file_put_contents($f,$s); }
'

echo "==> 6) Sidebar entries (outside Settings)"
SIDEBAR="resources/views/partials/sidebar.blade.php"
if [ -f "$SIDEBAR" ]; then
  cp -a "$SIDEBAR" "$SIDEBAR.bak.$TS"
  perl -0777 -i -pe '
    my $add = qq{\n  <a href="{{ route(\'countries.index\') }}" class="block rounded-lg border p-3 bg-white hover:bg-gray-50 mt-2">Countries</a>\n  <a href="{{ route(\'networks.index\') }}" class="block rounded-lg border p-3 bg-white hover:bg-gray-50 mt-2">Networks</a>\n};
    if ($_ !~ /route\(\s*\'countries\.index\'\s*\)/) { s{</div>\s*</div>\s*</x-app-layout>|$}{$add$&}s }
  ' "$SIDEBAR" || true
fi

echo "==> 7) Artisan importer (Google AOSP)"
...
class ImportCarriers extends Command {
  ...
  public function handle(){
    ...
    // --- Download helpers (AOSP raw returns base64 when ?format=TEXT) ---
    $get = function(string $url){
      ...
      // if looks like base64, decode (AOSP ?format=TEXT)
      $dec = base64_decode($raw, true);
      return $dec!==false ? $dec : $raw;
    };

    // --- 1) Countries + MCCs from MccTable.java ---
    ...
    // pattern: sTable.add(new MccEntry(202,"gr",2)); //Greece
    $n = 0;
    ...
    // --- 2) Networks from carrier_list.textpb (TelephonyProvider) ---
    ...
    // Split into carrier blocks by "carriers { ... }" or "carrier { ... }"
    $blocks = preg_split('/\\n(?=\\s*(carriers?|carrier)\\s*\\{)/', $carTxt);
    $imported = 0;
    foreach ($blocks as $b) {
      if (!preg_match('/mccmnc\\s*:\\s*"([0-9]{5,6})"/', $b)) continue;
      // name fields (varies by version): canonical_name, name, carrier_name
      $name = null;
...
if [ -f "$KERNEL" ]; then
  cp -a "$KERNEL" "$KERNEL.bak.$TS"
  perl -0777 -i -pe "s#protected \\$commands = \\[#protected \$commands = [\\n        \\\\App\\\\Console\\\\Commands\\\\ImportCarriers::class,# unless /ImportCarriers::class/" "$KERNEL"
fi

echo "==> 8) Migrate & cache"
...
echo "==> 9) Initial import from Google AOSP (fresh)"
$DC exec -T app bash -lc 'php artisan carriers:import --fresh'

echo "==> 10) Done. Open /countries and /networks"
